...
The patch follows the existing MyVesta web dialect and does not replace the panel. It adds a native-style `PHP` action immediately before `WordPress` in the domain list for administrators and domain owners. The configuration form follows MyVesta's standard one-column edit layout, stores settings outside generated pool files, reapplies them after web rebuilds, and validates the selected PHP-FPM service before restarting it.
