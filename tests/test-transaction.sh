...
# The single-quoted lines are literal contents of the generated mock script.
...
"$ROOT/bin/v-update-domain-php-config" example.test yes set memory_limit 384M set pm.max_children 7 > "$TEST_ROOT/command-output"
...
# The PHP snippet receives shell data only through argv.
# shellcheck disable=SC2016
php -r '
...
"$ROOT/bin/v-update-domain-php-config" example.test no reset memory_limit > "$TEST_ROOT/command-output"
! grep -q '^memory_limit=' "$MYVESTA_DOMAIN_PHP_STATE/domains/example.test.conf"
[[ "$(grep -c '^php_admin_value\[memory_limit\] = 256M$' "$POOL")" -eq 1 ]]
! grep -q '^php_admin_value\[memory_limit\] = 384M$' "$POOL"

"$ROOT/bin/v-change-domain-php-config" example.test pm.max_requests 2000 no > "$TEST_ROOT/command-output"
...
if "$ROOT/bin/v-update-domain-php-config" example.test no set post_max_size 128M > "$TEST_ROOT/command-output" 2>&1; then
...
if "$ROOT/bin/v-update-domain-php-config" example.test yes set upload_max_filesize 64M > "$TEST_ROOT/command-output" 2>&1; then
